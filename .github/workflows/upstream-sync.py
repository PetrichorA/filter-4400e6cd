import os
from base64 import urlsafe_b64encode
from hashlib import sha3_384
from shutil import rmtree

from requests.models import Response
from requests.sessions import Session

salt = b''


def get_version_string(data: bytes, salt: bytes) -> str:
    """
    Generates a version string based on the given data and salt.

    Args:
        data (bytes): The data used to generate the hash.
        salt (bytes): The salt used to modify the hash.

    Returns:
        str: The URL-safe version string generated from the hash.

    """
    sha = sha3_384(data)
    sha.update(salt)
    for _ in range(64):
        sha.update(sha.digest())
        sha.update(salt)
    return urlsafe_b64encode(sha.digest()).decode()


def is_subdomain(base: str, other: str) -> bool:
    """
    Checks if a domain name is a subdomain of another domain name.

    Args:
        base (str): The base domain name.
        other (str): The other domain name to check.

    Returns:
        bool: True if other is a subdomain of base, otherwise False.

    """
    return other.endswith(base) and (
        len(other) == len(base) or
        other[-(len(base) + 1)] == '.'
    )


def sort_domains(domains: list, unblock_domains: list = []) -> list:
    """
    Sorts a list of domain names based on a custom sorting order.

    If unblock_domains is not empty, domain names that are subdomains of
    any of the domain names in unblock_domains will be prioritized.

    Args:
        domains (list): The list of domains to sort.
        unblock_domains (list, optional): The list of domains to prioritize.

    Returns:
        list: The sorted list of domain names.

    """
    def reverse_domain(domain: str) -> list:
        return list(reversed(domain.split('.')))

    cur = list(map(lambda domain: (domain, reverse_domain(domain)),
                   list(set(domains))))
    cur.sort(key=lambda tup: tup[1])

    if len(unblock_domains) == 0:
        return list(map(lambda tup: tup[0], cur))

    index = 0
    max_index = len(cur)
    result = []
    unblock_domains = sort_domains(unblock_domains)

    for dead_domain in unblock_domains:
        cur_domain = reverse_domain(dead_domain)
        while index < max_index and cur_domain > cur[index][1]:
            result.append(cur[index][0])
            index += 1
        while index < max_index and is_subdomain(dead_domain, cur[index][0]):
            index += 1
        if index >= max_index:
            break

    if index < max_index:
        result.extend(map(lambda tup: tup[0], cur[index:]))

    return result


def adapt_ublock(domains: list[str]) -> list[str]:
    """Adapts uBlock Origin's filter syntax to the provided list of domains."""
    res = [domains[0]]
    size = len(domains)
    for i in range(1, size):
        if not is_subdomain(res[-1], domains[i]):
            res.append(domains[i])
    return res


header_text = """
! Title: filter-4400e6cd / external
! Expires: 2 days
! Homepage: https://github.com/PetrichorA/filter-4400e6cd
! Description: 自用屏蔽规则（此规则适用于浏览器）
! Subscribe: https://filter-4400e6cd.pages.dev/external.txt
! Subscribe (jsDelivr): https://cdn.jsdelivr.net/gh/PetrichorA/filter-4400e6cd@main/external.txt
! Subscribe (Raw): https://raw.githubusercontent.com/PetrichorA/filter-4400e6cd/main/external.txt
! Version: {version}
""".strip()


session = Session()


def get(url: str) -> Response:
    """
    Sends a GET request to the provided URL using a shared session object.

    Args:
        url (str): The URL to send the request to.

    Returns:
        requests.Response: The response received from the server.
    """
    while True:
        try:
            resp = session.get(url, timeout=3.0)
            if content_length := resp.headers.get('content-length'):
                assert str(len(resp.content)) == content_length
            return resp
        except Exception as err:
            print('error', err)


def transform_hosts(rules: str) -> str:
    """
    Transforms the provided rules into a version that can be used with uBlock Origin.

    Args:
        rules (str): The rules to transform.

    Returns:
        str: The transformed rules.
    """
    return '\n'.join(map(
        lambda domain: f'||{domain}^',
        adapt_ublock(sort_domains(rules.splitlines()))))


def transform_strip_comments(rules: str) -> str:
    """
    Removes comments and certain types of rule syntax from the provided rules.

    Args:
        rules (str): The rules to transform.

    Returns:
        str: The transformed rules.
    """
    return '\n'.join(filter(lambda line: line and not line.startswith('!') and not line.startswith('['), rules.splitlines()))


trusted_3p = (
    ('https://pgl.yoyo.org/adservers/serverlist.php?hostformat=nohtml&showintro=0',
     transform_hosts),
    ('https://github.com/uBlockOrigin/uAssets/raw/master/filters/annoyances.txt',
     transform_strip_comments),
    ('https://secure.fanboy.co.nz/fanboy-cookiemonster.txt',
     transform_strip_comments),
    ('https://secure.fanboy.co.nz/fanboy-annoyance.txt',
     transform_strip_comments),
)


if __name__ == '__main__':
    working_directory = os.path.normpath(os.path.join(os.path.dirname(
        os.path.abspath(__file__)), '..', '..', 'external'))

    content = []

    for name in os.listdir(working_directory):
        if name.endswith('.txt'):
            name = os.path.join(working_directory, name)
            with open(name, 'rb') as fin:
                namestr = get_version_string(fin.read(), salt) + '.txt'
            ename = os.path.join(working_directory, namestr)
            if name != ename:
                os.rename(name, ename)
            content.append(namestr)

    urls_dir = os.path.join(working_directory, 'url')
    rmtree(path=urls_dir, ignore_errors=True)
    os.mkdir(urls_dir)

    for url, transformer in trusted_3p:
        name = get_version_string(url.encode(), salt) + '.txt'
        with open(os.path.join(urls_dir, name), 'w', encoding='utf-8') as fout:
            fout.write(transformer(get(url).text.strip()))
        content.append(f'url/{name}')

    content.sort()
    content = '\n'.join(
        map(lambda namestr: '!#include external/{}'.format(namestr), content))
    version = get_version_string(content.encode(), salt)

    with open(os.path.join(working_directory, '..', 'external.txt'), 'w', encoding='utf-8') as fout:
        fout.write(header_text.format(version=version))
        fout.write('\n\n')
        fout.write(content)
